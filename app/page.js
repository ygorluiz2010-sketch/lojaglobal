  ...
  return (
    <main style={{background:"#f5f5f5",minHeight:"100vh",fontFamily:"Arial"}}>
      ...
      <div style={{padding:"30px"}}>
        <div style={{background:"linear-gradient(to right,#ff5a00,#ff0000)",padding:"40px",borderRadius:"25px",color:"white",marginBottom:"30px"}}>
          <h2 style={{fontSize:"48px",fontWeight:"bold"}}>SUPER OFERTAS</h2>
          <p>Produtos estilo Shopee e AliExpress</p>
        </div>

        <div style={{display:"grid",gridTemplateColumns":"repeat(auto-fit,minmax(220px,1fr))",gap:"20px"}}>
          {produtos.map((p, i) => (
            <div key={i} style={{background:"white",padding:"15px",borderRadius:"20px",boxShadow:"0 4px 10px rgba(0,0,0,0.1)"}}>
              <div style={{height:"180px",background:"#ddd",borderRadius:"15px",marginBottom:"15px"}}></div>
              <h2>{p.nome}</h2>
              <p style={{color:"#ff5a00",fontSize:"24px",fontWeight:"bold"}}>
                R$ {p.preco.toFixed(2)}
              </p>
              <p>Alemanha: € {(p.preco * euroRate).toFixed(2)}</p>
              <p style={{fontSize:"12px",color:"#666"}}>{p.vendido}</p>
